...
class Demo(Robot):  # Create a Robot
    ...
    def run(self):  # NECESARY FOR THE GAME  main loop to command the bot
        ...
        self.radarTurn(
            180
        )  # to turn the radar (negative values turn counter-clockwise)
        self.stop()
    ...
    def sensors(self):  # NECESARY FOR THE GAME
        ...
        angle = (
            self.getGunHeading()
        )  # Returns the direction that the robot's gun is facing
        angle = self.getHeading()  # Returns the direction that the robot is facing
        angle = (
            self.getRadarHeading()
        )  # Returns the direction that the robot's radar is facing
    ...
    def onRobotHit(self, robotId, robotName):  # when My bot hit another
        self.rPrint(
            "collision with:" + str(robotName)
        )  # Print information in the robotMenu (click on the righ panel to see it)

    def onHitByBullet(
        self, bulletBotId, bulletBotName, bulletPower
    ):  # NECESARY FOR THE GAME
